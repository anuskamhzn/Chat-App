...
const deleteProfileController = async (req, res) => {
  try {
    ...
    // Emit real-time update to all clients in global room
    if (global.io) {
      global.io.to('global').emit('userDeleted', { userId: req.user.id });
    }
    ...
    return res.status(200).json({
      success: true,
      message: 'Profile deleted successfully. Messages anonymized as "Deleted User".',
    });
  } catch (error) {
